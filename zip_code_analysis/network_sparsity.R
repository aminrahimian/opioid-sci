...
#saveRDS(lw_2_entire_us, file="lw_2_entire_us.rds")
spatial_autocorrelation <- sacsarlm(deaths_per_capita ~ deaths_social_porximity + deaths_spatial_proximity+
                                        ACS_PCT_HU_NO_VEH+
                                        POS_MEAN_DIST_ALC+ACS_PCT_OTHER_INS+
                                        ACS_PCT_LT_HS+AHRF_TOT_COM_HEALTH_GRANT+ACS_MEDIAN_HH_INC+
                                        +CCBP_BWLSTORES_RATE+AMFAR_MHFAC_RATE
                                      +ODR+ Naloxone_Available +Buprenorphine_Available+St_count_illicit_opioid_reported, 
                                      data=cdc_mort_data_fips_wise_death_certificates,
                                      listw = lw_2_entire_us,listw2=lw_1_entire_us,
                                      zero.policy = TRUE,
                                      tol.solve = 1*exp(-50)
)
...
formula_ivreg <- y ~ X_n + s_i + d_i |Q_n

# Fit the SARAR(2,1) model using ivreg
sarar_ivreg_model <- ivreg::ivreg(y ~ X_n + s_i + d_i |Q_n,weights = population)

# Check the summary of the SARAR model estimated by ivreg
summary(sarar_ivreg_model)

summary(sarar_ivreg_model, vcov = sandwich, diagnostics = TRUE)
# You may want to check for diagnostics as well, such as:
# - Endogeneity of the spatial lags
# Ensure all variables are numeric

summary_ivreg <- summary(sarar_ivreg_model, diagnostics = TRUE)
...
residuals <- residuals(sarar_ivreg_model)

# Create a Q-Q plot
qqnorm(residuals)
qqline(residuals, col = "red")  # A
### different values of s_i and d_i###
# Example usage
independent_vars <- c("deaths_spatial_proximity","ACS_PCT_HU_NO_VEH", "POS_MEAN_DIST_ALC", "ACS_PCT_OTHER_INS", "ACS_PCT_LT_HS", 
                      "AHRF_TOT_COM_HEALTH_GRANT", "ACS_MEDIAN_HH_INC", "CCBP_BWLSTORES_RATE", 
                      "AMFAR_MHFAC_RATE", "ODR", "Naloxone_Available", "Buprenorphine_Available", 
                      "St_count_illicit_opioid_reported")
regression_coefficients <- calculate_regression_coefficients(results, cdc_mort_data_fips_wise_death_certificates, independent_vars)
...
### numerator of w_i_j: population_i*cumulative_sci_weighted_i
w_i_j_test <- w_i_j
w_i_j_values <- as.vector(w_i_j_test)
### chatgpt create a function  for getting quantile from 5% to 25% 
#### update and store w_i_j based on the condition w_i_j < 5%

w_i_j_test[w_i_j_test < 0.00035445 ] <- 0
w_i_j_test <- w_i_j_test/rowSums(w_i_j_test)
rowSums(w_i_j_test)
#### further calculating s_{-i}
v <- social_df$deaths_per_capita
for(i in 1:ncol(w_i_j_test)){
  w_i_j_test[,i] <- w_i_j_test[,i] * v[i]
}

#### updated_ s_minus_i_mean ###
s_minus_i_mean <- rowSums(w_i_j_test)
...
a_i_j <- a_i_j / normalised_scale
a_i_j_test <- a_i_j
a_i_j_test[a_i_j_test < 0.0005562145 ] <-0
a_i_j_test <- a_i_j_test/rowSums(a_i_j_test)
# Perform matrix multiplication for deaths in spatial proximity ###
d_minus_i_test <- a_i_j_test %*% y

m_1_5<- lm(deaths_per_capita ~ s_minus_i_mean + d_minus_i_test+
     ACS_PCT_HU_NO_VEH+
     POS_MEAN_DIST_ALC+ACS_PCT_OTHER_INS+
     ACS_PCT_LT_HS+AHRF_TOT_COM_HEALTH_GRANT+ACS_MEDIAN_HH_INC+
     +CCBP_BWLSTORES_RATE+AMFAR_MHFAC_RATE
   ...
   data = cdc_mort_data_fips_wise_death_certificates, 
   weights = population)
m_11_5<- coeftest(m_1, vcov = vcovCL,cluster = ~ cdc_mort_data_fips_wise_death_certificates$stnchsxo)
m_11_5

m_2_1<- lm(deaths_per_capita ~ s_minus_i_mean + d_minus_i_test+
             ACS_PCT_HU_NO_VEH+
             POS_MEAN_DIST_ALC+ACS_PCT_OTHER_INS+
             ACS_PCT_LT_HS+AHRF_TOT_COM_HEALTH_GRANT+ACS_MEDIAN_HH_INC+
             +CCBP_BWLSTORES_RATE+AMFAR_MHFAC_RATE
           ...
           data = cdc_mort_data_fips_wise_death_certificates, 
           weights = population)
m_22_1<- coeftest(m_1, vcov = vcovCL,cluster = ~ cdc_mort_data_fips_wise_death_certificates$stnchsxo)
m_22_1

#####

calculate_regression_with_thresholds <- function(w_i_j, a_i_j, w_threshold, a_threshold, data, independent_vars) {
  # Apply threshold and normalize w_i_j
  w_i_j_test <- w_i_j
  w_i_j_test[w_i_j_test < w_threshold] <- 0
  w_i_j_test <- w_i_j_test / rowSums(w_i_j_test)
  
  # Calculate s_minus_i
  v <- data$deaths_per_capita
  for (i in 1:ncol(w_i_j_test)) {
    w_i_j_test[, i] <- w_i_j_test[, i] * v[i]
  }
  s_minus_i_mean <- rowSums(w_i_j_test)
  
  # Apply threshold and normalize a_i_j
  a_i_j_test <- a_i_j
  a_i_j_test[a_i_j_test < a_threshold] <- 0
  a_i_j_test <- a_i_j_test / rowSums(a_i_j_test)
  
  # Calculate d_minus_i
  d_minus_i_test <- a_i_j_test %*% data$deaths_per_capita
  
  # Construct the linear model
  data_for_model <- cbind(data, s_minus_i_mean, d_minus_i_test)
  formula <- as.formula(paste("deaths_per_capita ~ s_minus_i_mean + d_minus_i_test +", paste(independent_vars, collapse=" + ")))
  lm_model <- lm(formula, data = data_for_model, weights = data$population)
  
  # Coefficient test
  coeftest_result <- coeftest(lm_model, vcov = vcovCL, cluster = ~data$stnchsxo)
  
  return(list(model = lm_model, coefficients = coeftest_result))
}

w_threshold = 3.142354e-06  ### 5% quantile
a_threshold = 9.82443e-05  ### 5% quantile
independent_vars = c("ACS_PCT_HU_NO_VEH", "POS_MEAN_DIST_ALC", "ACS_PCT_OTHER_INS", "ACS_PCT_LT_HS", "AHRF_TOT_COM_HEALTH_GRANT", "ACS_MEDIAN_HH_INC", "CCBP_BWLSTORES_RATE", "AMFAR_MHFAC_RATE", "ODR", "Naloxone_Available", "Buprenorphine_Available", "St_count_illicit_opioid_reported")
result <- calculate_regression_with_thresholds(w_i_j, a_i_j, w_threshold, a_threshold, cdc_mort_data_fips_wise_death_certificates, independent_vars)


#### result _2 
w_threshold = 1.437785e-06 ### 10% quantile
a_threshold = 0.0001221617 ### 10% quantile ###
independent_vars = c("ACS_PCT_HU_NO_VEH", "POS_MEAN_DIST_ALC", "ACS_PCT_OTHER_INS", "ACS_PCT_LT_HS", "AHRF_TOT_COM_HEALTH_GRANT", "ACS_MEDIAN_HH_INC", "CCBP_BWLSTORES_RATE", "AMFAR_MHFAC_RATE", "ODR", "Naloxone_Available", "Buprenorphine_Available", "St_count_illicit_opioid_reported")
result_2 <- calculate_regression_with_thresholds(w_i_j, a_i_j, w_threshold, a_threshold, cdc_mort_data_fips_wise_death_certificates, independent_vars)


#### result_3
w_threshold = 4.99133e-06  ### 15% quantile
a_threshold = 0.0001387414  ### 15% quantile ###
independent_vars = c("ACS_PCT_HU_NO_VEH", "POS_MEAN_DIST_ALC", "ACS_PCT_OTHER_INS", "ACS_PCT_LT_HS", "AHRF_TOT_COM_HEALTH_GRANT", "ACS_MEDIAN_HH_INC", "CCBP_BWLSTORES_RATE", "AMFAR_MHFAC_RATE", "ODR", "Naloxone_Available", "Buprenorphine_Available", "St_count_illicit_opioid_reported")
result_3 <- calculate_regression_with_thresholds(w_i_j, a_i_j, w_threshold, a_threshold, cdc_mort_data_fips_wise_death_certificates, independent_vars)

#### result_4###3
w_threshold = 7.037654e-06   ### 15% quantile
a_threshold = 0.0001516215   ### 15% quantile ###
independent_vars = c("ACS_PCT_HU_NO_VEH", "POS_MEAN_DIST_ALC", "ACS_PCT_OTHER_INS", "ACS_PCT_LT_HS", "AHRF_TOT_COM_HEALTH_GRANT", "ACS_MEDIAN_HH_INC", "CCBP_BWLSTORES_RATE", "AMFAR_MHFAC_RATE", "ODR", "Naloxone_Available", "Buprenorphine_Available", "St_count_illicit_opioid_reported")
result_4 <- calculate_regression_with_thresholds(w_i_j, a_i_j, w_threshold, a_threshold, cdc_mort_data_fips_wise_death_certificates, independent_vars)

#### result 5######
w_threshold = 9.33894e-06    ### 25% quantile
a_threshold = 0.0001627387   ### 25% quantile ###
independent_vars = c("ACS_PCT_HU_NO_VEH", "POS_MEAN_DIST_ALC", "ACS_PCT_OTHER_INS", "ACS_PCT_LT_HS", "AHRF_TOT_COM_HEALTH_GRANT", "ACS_MEDIAN_HH_INC", "CCBP_BWLSTORES_RATE", "AMFAR_MHFAC_RATE", "ODR", "Naloxone_Available", "Buprenorphine_Available", "St_count_illicit_opioid_reported")
result_5 <- calculate_regression_with_thresholds(w_i_j, a_i_j, w_threshold, a_threshold, cdc_mort_data_fips_wise_death_certificates, independent_vars)


#### result 6 ##
w_threshold = 2.898697e-05     ### 50% quantile
a_threshold = 0.0002222093   ### 25% quantile ###
independent_vars = c("ACS_PCT_HU_NO_VEH", "POS_MEAN_DIST_ALC", "ACS_PCT_OTHER_INS", "ACS_PCT_LT_HS", "AHRF_TOT_COM_HEALTH_GRANT", "ACS_MEDIAN_HH_INC", "CCBP_BWLSTORES_RATE", "AMFAR_MHFAC_RATE", "ODR", "Naloxone_Available", "Buprenorphine_Available", "St_count_illicit_opioid_reported")
result_6 <- calculate_regression_with_thresholds(w_i_j, a_i_j, w_threshold, a_threshold, cdc_mort_data_fips_wise_death_certificates, independent_vars)

# Assuming result, result_2, result_3, result_4, result_5, result_6 are already defined
a <- result[[2]]
b <- result_2[[2]]
c <- result_3[[2]]
d <- result_4[[2]]
...
sparsity_check <- modelplot(list(a,b,c,d,e,f), coef_omit = -c(2,3), draw=TRUE)
...
sparsity_check <- sparsity_check + 
  ...
  scale_color_manual(labels = c("0.05 quantile threshold",
                                "0.10 qauntile threshold", "0.15 qauntile threshold", "0.20 qauntile threshold", "0.25 qauntile threshold", "0.5 qauntile threshold"), 
                     values = c("#fde725","#7ad151","#22a884","#2a788e","#414487", "#440154" ))
