##### This R script  is for spatial error regression for incident data##
...
data <- read.csv('incident_data_pa_gov.csv')
data <- data[,-1]
...
##### spatial regression ####
library(spatialreg)
serrRslt_incident_data <- spatialreg::errorsarlm(deaths_per_capita ~ log(county_deaths_social_proximity) +
                                   ...
                                   zero.policy = TRUE, 
                                   na.action = na.omit)
summary(serrRslt_incident_data)
