...
scaled_health_determinant <- nvss_ood_county_wise_2013_2017[,c(10:25)]
scaled_health_determinant <- scale(scaled_health_determinant)
nvss_ood_county_wise_2013_2017 <- nvss_ood_county_wise_2013_2017[,-c(10:25)]
...
##############################
x <- nvss_ood_county_wise_2013_2017[,c(10:25)]
x <- as.matrix(x)
...
x_test <- x[-index, ]
y_test <- y[-index]
cv.fit <- cv.glmnet(x_train, y_train, alpha = 1, nfolds = 6)
plot(cv.fit)
